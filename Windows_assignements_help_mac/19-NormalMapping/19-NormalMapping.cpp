...
GLuint uNormalMapSamplerUniform = 0;
...
int initialize(void)
{
	...
	const GLchar* vertexShaderCode =
		...
		"void main(void)" \
		"{" \
		"   oFragPos = vec3(uModelViewMatrix * aPosition);"
		"if(uKeyPress == 1)"
		"{"
	...
	glBindAttribLocation(shaderProgramObject, 4, "aTangent");
	...
	uNormalMapSamplerUniform = glGetUniformLocation(shaderProgramObject, "uNormalMapSampler");
	...
	normalTexture = createTexture2D("Stone.png");
	...
	light[0].diffuse = vec3(0.8f, 0.8f, 0.8f);
	light[1].diffuse = vec3(0.8f, 0.8f, 0.8f);
	
	light[0].specular = vec3(0.8f, 0.8f, 0.8f);
	light[1].specular = vec3(0.8f, 0.8f, 0.8f);
...
void display(void)
{
	...
	glUniform1i(uNormalMapSamplerUniform, 0);
