    package com.hvs.twoshapescolor;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener
{
    ...
    private void initialize(GL10 gl)
    {
        ...
        //VBO for Color Square
        GLES32.glGenBuffers(1,VBO_Colored_Square,0);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER,VBO_Colored_Square[0]);

        //prepare vbo color
        byteBuffer = ByteBuffer.allocateDirect(square_color.length*4);
        byteBuffer.order(ByteOrder.nativeOrder());
        FloatBuffer colorBuffer = byteBuffer.asFloatBuffer();
        positionBuffer.put(square_color);
        positionBuffer.position(0);

        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER,square_color.length*4,positionBuffer,GLES32.GL_STATIC_DRAW);
        ...
        //VBO for Color Triangle
        GLES32.glGenBuffers(1,VBO_Colored_Triangle,0);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER,VBO_Colored_Triangle[0]);
