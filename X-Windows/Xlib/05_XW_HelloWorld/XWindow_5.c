...
//code -------
    ...
    while (1)
    {
        ...
        case Expose://WM_Paint
                ...
                XSetFont(display,gc,pFontStruct->fid);                     //for exact color
                XAllocNamedColor(display, colormap, "green", &greenColor, &greenColor);
                XSetForeground(display, gc, greenColor.pixel);

                stringLength = strlen(str);
                stringWidht = XTextWidth(pFontStruct, str, stringLength);
                fontHeight = pFontStruct->ascent + pFontStruct->descent;

                XDrawString(display, window, gc, (winWidth - stringWidht)/2, (winHeight - fontHeight)/2, str, stringLength);
