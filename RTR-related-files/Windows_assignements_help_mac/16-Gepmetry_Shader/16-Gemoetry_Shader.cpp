...
int initialize(void)
{
	...
	//Gemoetry shader
	const GLchar* gemoetryShaderCode =
		"#version 460 core" \
		"\n" \
		"layout(triangles) in;" \
	...
	gemoetryShaderObject = glCreateShader(GL_GEOMETRY_SHADER);
	glShaderSource(gemoetryShaderObject, 1, (const GLchar**)&gemoetryShaderCode, NULL);
	glCompileShader(gemoetryShaderObject);

	status = 0;
	infoLogLength = 0;
	szInfoLog = NULL;

	glGetShaderiv(gemoetryShaderObject, GL_COMPILE_STATUS, &status);
	if (status == GL_FALSE)
	{
		glGetShaderiv(gemoetryShaderObject, GL_INFO_LOG_LENGTH, &infoLogLength);
		if (infoLogLength > 0)
		{
			szInfoLog = (GLchar*)malloc(infoLogLength);
			if (szInfoLog != NULL)
			{
				glGetShaderInfoLog(gemoetryShaderObject, infoLogLength, NULL, szInfoLog);
				fprintf(gpFILE, "Gemoetry shader compilation error log: %s\n", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
				uninitialize();
			}
	...
	glAttachShader(shaderProgramObject, gemoetryShaderObject);
