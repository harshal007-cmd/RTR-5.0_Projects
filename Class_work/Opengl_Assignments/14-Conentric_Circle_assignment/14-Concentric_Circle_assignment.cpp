...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("HVS:Concentric Triangle assignment"),
...
void Concentric_Circle(float radius, int count)
{
	float x, y, angle;
	float xCenter = 0.0;
	float yCenter = 0.0;
	...
	for (int i = 0;i < 10;++i)
	{
		if (i == 0)
			...
			glColor3f(1.0, 1.0, 1.0);//cy
		else if (i == 4)
			glColor3f(1.0, 1.0, 0.0);//y
		else if (i == 5)
			glColor3f(1.0, 1.0, 1.0);//w
		else if (i == 6)
			glColor3f(1.0, 0.5, 0.0);//o
		else if (i == 7)
			glColor3f(0.5, 0.2, 0.46);//m
		else if (i == 8)
			glColor3f(0.5, 0.5, 0.5);//lg
		else if (i == 9)
			glColor3f(1.0, 0.0, 1.0);//p
		
		
		glBegin(GL_LINE_LOOP);
		for (int j = 0;j < count; j++)
		{
			angle = 2.0f * M_PI * (float)j / float(count);
			x = radius * cos(angle);
			y = radius * sin(angle);
			glVertex2f((-1) * x + xCenter, (-1) * y + yCenter);
		}
		glEnd();
		radius -= 0.1;
	}
