...
PFNGLMULTITEXCOORD2FARBPROC glMultiTextCoord2f = NULL;
...
int initialize(void)
{
	...
	glMultiTextCoord2f = (PFNGLMULTITEXCOORD2FARBPROC)wglGetProcAddress("glMultiTexCoord2fARB");
	...
	glClearColor(1.0f, 0.5f, 0.0f, 1.0f); //here OpenGL starts

	bResult = loadGLTexture(&texture_smiley, MAKEINTRESOURCE(MY_SMILEY_BITMAP));
	if (bResult == FALSE)
	{
		fprintf(gpFILE, "Loadding of Smiley Texture failed\n");
		return -6;

	}
	bResult = loadGLTexture(&texture_smiley, MAKEINTRESOURCE(MY_STONE_BITMAP));
	if (bResult == FALSE)
	{
...
void display(void)
{
	...
	//2.Following call to be done when only Viewing x-formation is done
	...
	glBegin(GL_QUADS);
	//front face
	glMultiTextCoord2f(GL_TEXTURE1, 1.0, 1.0);
	glMultiTextCoord2f(GL_TEXTURE0, 1.0, 1.0);
	glVertex3f(1.0, 1.0, 0.0);

	glMultiTextCoord2f(GL_TEXTURE1, 0.0, 1.0);
	glMultiTextCoord2f(GL_TEXTURE0, 0.0, 1.0);
	glVertex3f(-1.0, 1.0, 0.0);

	glMultiTextCoord2f(GL_TEXTURE1, 0.0, 0.0);
	glMultiTextCoord2f(GL_TEXTURE0, 0.0, 0.0);
	glVertex3f(-1.0, -1.0, 0.0);

	glMultiTextCoord2f(GL_TEXTURE1, 1.0, 0.0);
	glMultiTextCoord2f(GL_TEXTURE0, 1.0, 0.0);
	glVertex3f(1.0, -1.0, 0.0);
