...
int initialize(void)
{
	...
	texture_icon = createTexture2D("icon.png");
	resize(WIDTH, HEIGHT);
	return 0;
}
...
GLuint creatTexture2D(const char* filePath)
{
	int w, h, channel;
	unsigned char* data = stbi_load(filePath, &w, &h, &channel, 0);
	if (!data)
	{
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, texture_icon);
