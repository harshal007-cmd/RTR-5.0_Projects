...
BOOL loadGLTexture(GLuint* texture, TCHAR imageResoureceID[])
{
	//local variable declarations
	HBITMAP hBitmap = NULL;
	BITMAP bmp;

	hBitmap = (HBITMAP)LoadImage(GetModuleHandle(NULL), imageResoureceID, IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
	if (hBitmap == NULL)
	{
	...
	//create opegl texture obj
	glGenTextures(1, texture);

	//bind to generated texture
	glBindTexture(GL_TEXTURE_2D, *texture);

	//unpack and aligment image data loaded by loadimage
...
void display(void)
{
	...
	glVertex3f(0.0f, 1.0f, 0.0f);//apex upper
