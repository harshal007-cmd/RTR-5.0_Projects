...
GLfloat pyramid[] = {
	...
	//Right Face
	0.0f, 1.0f, 0.0f, 1.0,0.0,0.0,//apex upper

	1.0f, -1.0f, 1.0f, 0.0,1.0,0.0,//LB

	1.0f, -1.0f, -1.0f, 0.0,0.0,1.0,//RB
...
int initialize(void)
{
	...
	//For vertex array concept
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_COLOR_ARRAY);

	glVertexPointer(3, GL_FLOAT, sizeof(GLfloat) * 6, pyramid);
	glColorPointer(3, GL_FLOAT, sizeof(GLfloat) * 6, &pyramid[3]);
...
void display(void)
{
	...
	//glBegin(GL_POINTS);
	for (int i = 0;i < ArrayLen(pyramid);i += 6)
	{
		glColor3f(pyramid[i + 3], pyramid[i + 4], pyramid[i + 5]);
		glVertex3f(pyramid[i], pyramid[i + 1], pyramid[i + 2]);
	}
	*/
	glDrawArrays(GL_TRIANGLES, 0, ArrayLen(pyramid) / 6);
